#!/usr/bin/env python
##
# Extracts the documentation from the latest tag of a branch.
##
import sys
import os
import zipfile
...
project = 'pg-python'
docpath = ('src', 'documentation', 'html') # inside the pg-python archive
...
def publish_branch(target, version, d = os.path.join(script_dir, 'tmpd')):
	if os.path.exists(d):
		rmrf(d)
	extract(d, open_branch(version))
	os.rename(os.path.join(d, *docpath), target)
	# remove tmpd
	rmrf(d)
